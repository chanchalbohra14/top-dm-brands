...
export const CharacterAnimation = ({ brand, isActive, onClick }: CharacterAnimationProps) => {
  ...
  return (
    ...
    >
      ...
      <div className={`relative transition-all duration-500 ${
        ...
        {isActive && (
          <div className={`absolute left-0 top-1/2 w-full h-0.5 bg-gradient-to-r from-transparent via-${brand.color} to-transparent animate-slide-in-right opacity-50`} />
        )}
      </div>
      
      {/* Character Speech Bubble */}
      {isHovered && (
        <div className={`absolute -top-16 left-1/2 transform -translate-x-1/2 bg-card backdrop-blur-glass border border-${brand.color}/30 rounded-lg px-4 py-2 text-sm font-medium animate-fade-in-up shadow-premium`}>
          <div className="text-center">
            <span className={`text-${brand.color}`}>{brand.name}</span>
            <div className={`absolute top-full left-1/2 transform -translate-x-1/2 w-0 h-0 border-l-4 border-r-4 border-t-4 border-transparent border-t-${brand.color}/30`} />
          </div>
        </div>
      )}
    </div>
